...
class RadiationPayload final : public RadiationPayloadComponentBase {
  ...
  private:
    ...
    // Protocol helpers — MOSAIC sends ASCII records delimited by <GAMMA_START>...<GAMMA_END>
    bool accumulateProtocolData(const U8* data, U32 size);
    void processRecords();
    void clearProtocolBuffer();
    bool writeChunkToFile(const U8* data, U32 size);
    void writeCompletedRecord(U32 recordSize);
    I32 findMarker(const U8* needle, U32 needleLen, U32 searchFrom = 0) const;
  ...
  public:
    ...
    // and open the first output file.
    void configure();
  ...
  private:
    ...
    U32 m_totalBytesReceived = 0;
    U32 m_chunksReceived = 0;

    Os::File m_file;
    bool m_fileOpen = false;
    char m_currentFilename[64] = {};

    // Protocol buffer accumulates incoming bytes until a complete <GAMMA_START>..<GAMMA_END>
    // record is present. Sized to hold several typical records (~75 bytes each).
